...
> run (a re-run would start a new login and invalidate the code), so run the first login interactively;
> the session is then cached in `.eufy-session.json` and later runs skip it. Run after `npm run build`.
