 ...
 *
 * One member, and the smallness is the finding rather than a gap. The captured unit (a T87A0,
 ...
 * get another way. The rest are named in {@link DISPLAY_PARAMS} — which is what makes them readable off
 * `getProperties()` — and deliberately have no typed getter:
 *
 ...
 *    would offer a caller a second spelling of what it just read.
 *  - 8003 is version-shaped and nothing confirms what it means, so its dictionary entry is `guessed` —
 *    and a guess does not belong in the typed surface, where a caller cannot see the label.
 *
...
export const DISPLAY_PARAM = { BATTERY: 8001 } as const;

/**
 * The `display` read. Read-only; see the module note for why there is no write.
 *
 * Exported but NOT published: the entry states its wire id and what the claim rests on, which the
...
export const DISPLAY_MEMBERS = {
   ...
   * id space — two wires that happen to mean the same thing. Putting 8001 on the security module would
   * have re-opened the door this line was split to close, so a consumer reads a display's charge through
   ...
   * eufy battery this SDK models is 0-100, and the capture said `"100"`.
   */
  battery: {
    ...
    kind: "percent",
    provenance: "mega",
    description: "Battery level (param 8001). The scale follows the line's convention.",
  },
